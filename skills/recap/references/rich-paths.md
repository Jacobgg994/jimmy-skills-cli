...
# Focus file
FOCUS_FILE="$ROOT/Jimmy/inbox/focus-agent-main.md"
...
# Activity log
ACTIVITY_LOG="$ROOT/Jimmy/memory/logs/activity.log"
[ -f "$ACTIVITY_LOG" ] && LAST_ACTIVITY=$(tail -1 "$ACTIVITY_LOG" | cut -d'|' -f3 | xargs)

# Latest retro
LATEST_RETRO=$(find "$ROOT/Jimmy/memory/retrospectives" -name "*.md" -type f 2>/dev/null | grep -v CLAUDE | sort -r | head -1)
```
...
Hot tracks: [from Jimmy/inbox/tracks/ or "None"]
...
HANDOFF=$(ls -t "$ROOT/Jimmy/inbox/handoff/"*.md 2>/dev/null | head -1)
...
# Get hot tracks
ls "$ROOT/Jimmy/inbox/tracks/"*.md 2>/dev/null | head -3
```
