...
// Resolve Jimmy symlink (used for focus + schedule)
const psi = existsSync("Jimmy") ? realpathSync("Jimmy") : "Jimmy";
...
const monthDir = `Jimmy/memory/retrospectives/${new Date().toISOString().slice(0, 7)}`;
...
try {
  const handoffs = (await $`ls -t Jimmy/inbox/handoff/*.md 2>/dev/null`.text()).trim().split('\n');
