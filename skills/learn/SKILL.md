...
/learn [slug]            # Use slug from Jimmy/memory/slugs.yaml
/learn [repo-path]       # Path to repo
/learn [repo-name]       # Finds in Jimmy/learn/owner/repo
...
```bash
unlink Jimmy/learn/owner/repo/origin   # Remove symlink
ghq rm owner/repo                  # Remove source
# Docs remain in Jimmy/learn/owner/repo/
```
...
Example: ROOT=/home/user/ghq/.../my-Jimmy, learning acme-corp/cool-library, TODAY=2026-02-04, TIME=1349:
```
READ from:  .../Jimmy/learn/acme-corp/cool-library/origin/
WRITE to:   .../Jimmy/learn/acme-corp/cool-library/2026-02-04/1349_[FILENAME].md
```
...
**Verify:**
```bash
ls -la "$ROOT/Jimmy/learn/$OWNER/$REPO/"
```

> **Note**: Grep tool doesn't follow symlinks. Use: `rg -L "pattern" Jimmy/learn/owner/repo/origin/`

### Then resolve path:
```bash
# Find by name (searches origin symlinks)
find Jimmy/learn -name "origin" -type l | xargs -I{} dirname {} | grep -i "$INPUT" | head -1
```

## Scope

**For external repos**: Clone with script first, then explore via `origin/`
**For local projects** (in `specs/`, `Jimmy/lib/`): Read directly
...
- DOCS_DIR = .../Jimmy/learn/acme-corp/cool-library/2026-02-04/
...
For Jimmys that want to commit docs but ignore symlinks:
