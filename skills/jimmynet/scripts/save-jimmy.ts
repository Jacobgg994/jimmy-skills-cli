...
async function getJimmy(nameOrSlug: string) {
...
if (!input) {
  console.error('Usage: bun save-Jimmy.ts \'{"name":"...","slug":"..."}\'')
  process.exit(1)
}
...
if (!existsSync(JimmyS_DIR)) {
  await mkdir(JimmyS_DIR, { recursive: true })
}

// Merge with existing (preserve bot_key if not provided)
const existing = await getJimmy(data.slug)
const merged = {
...
const filePath = join(JimmyS_DIR, `${merged.slug}.json`)
