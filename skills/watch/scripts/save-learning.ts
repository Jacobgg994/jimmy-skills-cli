...
//
// Creates markdown file in Jimmy/memory/learnings/
...
let learningFile = join(ROOT, `Jimmy/memory/learnings/${date}_${slug}.md`);
let count = 1;
while (existsSync(learningFile)) {
  learningFile = join(ROOT, `Jimmy/memory/learnings/${date}_${slug}-${count}.md`);
  count++;
}
