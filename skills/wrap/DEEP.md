...
mkdir -p "$ROOT/Jimmy/memory/retrospectives/$DATE_PATH"
```
...
- Read Jimmy/memory/logs/ for today
...
### Agent 5: Jimmy Memory Search
```
Search Jimmy for related context:
- Jimmy_search("[session focus]")
- Check Jimmy/memory/learnings/ for similar topics
...
**Location**: `Jimmy/memory/retrospectives/$DATE_PATH/${TIME}_[slug].md`
...
- Jimmy connections (from Agent 5)

## Step 3: Write Lesson Learned

**Location**: `Jimmy/memory/learnings/${TODAY}_[slug].md`
...
```bash
git add Jimmy/memory/retrospectives/ Jimmy/memory/learnings/
