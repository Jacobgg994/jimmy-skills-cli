...
```bash
unlink Jimmy/incubate/owner/repo
rmdir Jimmy/incubate/owner 2>/dev/null
...
git -C Jimmy/incubate/owner/repo checkout -b feat/feature-1
# ... work, commit, push, PR ...
git -C Jimmy/incubate/owner/repo checkout -b feat/feature-2
# ... work, commit, push, PR ...
...
find Jimmy/learn Jimmy/incubate -type l 2>/dev/null | grep -i "query"
```
...
find Jimmy/learn Jimmy/incubate -type l ! -exec test -e {} \; -print 2>/dev/null
```
...
```bash
# Add to learn
ghq get -u URL && mkdir -p Jimmy/learn/owner && ln -sf "$(ghq root)/github.com/owner/repo" Jimmy/learn/owner/repo

# Add to incubate
ghq get -u URL && mkdir -p Jimmy/incubate/owner && ln -sf "$(ghq root)/github.com/owner/repo" Jimmy/incubate/owner/repo

# Offload (remove symlink only)
unlink Jimmy/incubate/owner/repo && rmdir Jimmy/incubate/owner 2>/dev/null

# Offload + purge (remove symlink AND ghq clone)
unlink Jimmy/incubate/owner/repo && rm -rf "$(ghq root)/github.com/owner/repo"
